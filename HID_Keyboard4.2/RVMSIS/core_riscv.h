...
#define __nop()      asm volatile("nop")
         ...
         */
        RV_STATIC_INLINE void PFIC_SetFastIRQ(uint32_t addr, IRQn_Type IRQn,
                uint8_t num) {
            if (num > 3)
                return;
            PFIC->FIBADDRR = addr;
